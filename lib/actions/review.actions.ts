...
import { detectFakeReview } from '../ai'
...
export async function createUpdateReview({
  ...
  try {
    ...
    const { isFake, reason } = await detectFakeReview(review.comment)
    ...
    if (existReview) {
      ...
      existReview.isFake = isFake
      existReview.fakeReason = reason
      ...
      await Review.create({ ...review, isFake, fakeReason: reason })
      await updateProductReview(review.product)
      revalidatePath(path)
      return {
        success: true,
